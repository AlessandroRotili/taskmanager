...
<!-- 
	...
	che è stato riempito grazie al mapping /dettagli in HomeController.
...
<%	Task t = (Task) request.getAttribute("task");	%>
 <% List<ProjectOwner> POs = (List<ProjectOwner>) request.getAttribute("elencoProjectOwners"); %>   
<!DOCTYPE html>
	<html>
	<head>
...
.btn:active {
  ...
  box-shadow: none;
	...
	<!--
		Questa form è simile a quella per l'aggiunta di un nuovo libro
		ma oltre ad autore e titolo viene inserito in formato readonly anche l'id.
		Una volta cliccato il tasto aggiorna, l'utente verrà reindirizzato al mapping modificalibro portando con se la mappa del libro.
	-->
	
	<form action="/tasks/modificatask" method="get">
	<div class="main-form">
	     <h1 align="center">DETTAGLI DEL TASK</h1>
			ID <input type="text" name="id" value="<%= t.getId() %>" readonly><br>
			TITOLO <input type="text" name="titolo" value="<%= t.getTitolo() %>"><br>
			DIFFICOLTA' <select class="select" name="difficolta">
							<option value="Facile">Facile</option>
							<option value="Medio">Medio</option>
							<option value="Difficile">Difficile</option>
						</select>	
			<br>
			PROJECTOWNER <select class="select" name="idprojectowner">
								<% for(ProjectOwner d : POs) {%>
