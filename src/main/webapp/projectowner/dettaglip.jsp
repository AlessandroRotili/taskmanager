...
<!-- 	Andiamo a leggere il model che ci arriva dall'URL e lo trasformiamo in un oggetto di tipo
		Stanza-->
<% ProjectOwner p = (ProjectOwner) request.getAttribute("projectowner"); %>
<% List<TaskAssegnato> tasks = (List<TaskAssegnato>) request.getAttribute("elencoTasksAssegnati"); %>
<% List<Task> task = (List<Task>) request.getAttribute("elencoTask"); %>

<!DOCTYPE html>
<html>
	<head>
		<meta charset="ISO-8859-1">
		<title>Dettagli del projectowner con id <%= p.getId() %></title>
		<style>
		.styled-table {
...
.btn:active {
  ...
  box-shadow: none;
	...
	<body>
		<h1>Dettagli del Project Owner <%= p.getNome() %></h1>
		
		<p>NOME: <%= p.getNome() %></p><br>
		...
		<tbody>	    
			<% for(TaskAssegnato ta : tasks)
			{ %>
				<% for(Dipendente d : p.getDipendenti())
				{ %>
					<%for(Task t : task)
					{ %>
				    <%if(Integer.parseInt(ta.getIdDipendente())==d.getId() && ta.getIdTask()==t.getId() && t.getIdProjectOwner() == p.getId() ) {%>			
				<tr>
				    <td><%= d.getNome() %></td>
		...
		<a href="/homed"> <button class="btn" role="button">HOME</button></a>
	</body>
</html>