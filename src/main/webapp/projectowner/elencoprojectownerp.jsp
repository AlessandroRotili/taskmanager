<!--	Questa intestazione serve solo a specificare il tipo di codice che troviamo all'interno della
		pagina ossia JAVA. Una pagina JSP è una pagina HTML con del codice JAVA.
		Rispetto alle pagine classiche potrebbe risultare più scomoda perché quando cambio qualcosa
		al suo interno devo riavviare l'intera app.
		Ciò che la rende invece migliore è la sicurezza perché le operazioni al suo interno non
...
<!--	Per prima cosa imprtiamo List e Stanza per rendere la JSP
 ...
 <%@ page import="com.generation.taskmanager.entities.ProjectOwner" %>
 ...
 -->
 <% List<ProjectOwner> po = (List<ProjectOwner>) request.getAttribute("elencoProjectOwners"); %>

<!DOCTYPE html>
<html>
	<head>
		<meta charset="ISO-8859-1">
		<title>Elenco dei projectowners</title>
		<style>
		.styled-table {
...
.btn:active {
  ...
  box-shadow: none;
	...
	<body>
		...
		<h1>ELENCO DEI PROJECT OWNERS</h1>
		<% if(po.size() == 0){ %>
		...
		<table class="styled-table">
		  <thead>
			...
			<tbody>
			 <% for(ProjectOwner p : po) {%>
			 	<tr>
			 		<td><%= p.getId() %></td>
			 		<td><%= p.getNome() %></td>
			 		<td>
			 			<a href="/projectowner/dettagli?id=<%= p.getId() %>"><button class="button" role="button">Dettagli</button></a>
			 		</td>
			 		<td>
			 			<a href="/projectowner/eliminaprojectowner?id=<%= p.getId() %>"><button class="button" role="button">Elimina</button></a>
			 		</td>
			 	</tr>
			<%} %>
			</tbody>
		</table><br>
		<a href="/projectowner/formnuovo"><button class="btn" role="button">NUOVO PO</button></a>
